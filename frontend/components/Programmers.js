...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList.find(pr=>pr.id==selectedID).name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: selectedID===''? 'royalblue':'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setSelectedID(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
